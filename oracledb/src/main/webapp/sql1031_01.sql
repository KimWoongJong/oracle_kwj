-- 부서별 최대연봉, 최소연봉, 평균연봉을 출력하시오.
-- 평균 부서별 연봉이 5000 이상인 부서만 출력하시오.
select department_id,max(salary),min(salary),round(avg(salary),2) as stu_avg
from employees
where department_id>50 -- 일반함수 where절
group by department_id -- select 뒤에 일반함수와 그룹함수를 함께 입력할 경우 group by를 반드시 넣어야 함
having avg(salary)>=5000 -- 그룹함수 having절
...
select count(*) from employees, departments; -- 107*27=2889 의미없는 개수가 나옴
...
-- employees, jobs
-- 사원번호, 사원이름, 직급, 지급최대월급, 최소월급을 출력하시오.
select employee_id, emp_name, j.job_id,job_title, min_salary, max_salary
