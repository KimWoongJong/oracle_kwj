...
-- id: 외래키 - member primary key -> 중복안됨, null 없는 데이터 입력하게 되면 에러처리
insert into nboard values(
nboard_seq.nextval,'fff','제목6','내용6',sysdate
);
...
-- 그룹함수에서 그룹함수의 값의 조건을 처리하려면 having 명령어를 사용해야 함
select department_id, count(commission_pct)
from employees
group by department_id
...
-- 부서별 평균이 전체평균 이상인 부서만 출력하시오.
select department_id, avg(salary)
from employees
-- where department_id=90 -- where 뒤에 오는 것은 보고자 하는 column department_id에 대한 조건을 넣는 것 <- 단일함수에 대한 조건은 where로 
group by department_id
having avg(salary)>(select avg(salary) from employees) -- having 뒤에 오는 것은 결과값이 avg(salary)에 대한 조건을 넣는 것 <- 그룹함수에 대한 조건은 having으로
;

-- 부서별 평균이 전체평균 이상인 부서만 출력(+부서명)
